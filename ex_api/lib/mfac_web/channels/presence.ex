defmodule MfacWeb.Presence do
  use Phoenix.Presence, otp_app: :mfac,
                        pubsub_server: Mfac.PubSub
  ...
  """
  def fetch(_topic, entries) do
    ids = Enum.map(Map.keys(entries), fn(k) -> k end)
    query =
      from u in User,
        where: u.id in ^ids
    users = 
      query
      |> Repo.all
      |> Enum.into(%{}, fn(u) -> {Integer.to_string(u.id), MfacWeb.UserView.render("user_simple.json", user: u)} end)
