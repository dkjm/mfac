...
//import {SelectableList} from '../../hocs'
...
class NavDrawer extends Component {
	...
	handleRequestLogout = () => {
		console.log('handleRequestLogout')
	...
	makeNonAdminItems = () => {
		const items = [
		    ...
		    primaryText: 'Dashboard',
		    leftIcon: <Home />,
		    onClick: () => this.handleSelectItem('/dash'),
		    widget: ListItem,
		    value: 1,
		  },
		  {
	...
	renderNavItems() {
		...
		let items = []
		if (isAdmin) {
			items = this.makeAdminItems()
		}
		else {
			items = this.makeNonAdminItems()
		}
		...
		// can't get selectable list to work
		// after page refresh.
  ...
  render() {
  	const {navDrawerVisible} = this.props
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    navDrawerVisible: state.ui.navDrawerVisible,
  }
}
